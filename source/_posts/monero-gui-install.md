...
概述：本文将详细讲解Windows平台下Monero GUI钱包的完整安装、配置及使用流程，涵盖门罗币核心概念、钱包下载验证、安装配置、收发款操作等关键环节，兼顾新手友好性与安全性，同时补充技术细节和风险提示，帮助用户快速掌握门罗币钱包的使用方法，规避常见操作风险。
...
- **/monero-storage/wallets/***/wallet.key**：钱包密钥文件，存储钱包的私钥相关信息，是资产安全的核心，绝对不能泄露给他人，建议定期备份。
...
备份完成后，点击“下一步”，系统会要求重新输入助记词（验证备份是否正确），按照记录的助记词，逐字选择对应字符，输入完成后点击“下一步”（此处原文“输入空空”应为笔误，实际需准确输入助记词）。再次强调：OneKey Pro硬件钱包无需在电脑端输入助记词，全程在硬件设备上操作，安全性远超软件钱包，是资产存储的最优选择。
...
- **开启Prune模式**：勾选“Prune”选项，可开启 pruning 模式，该模式下仅同步区块链核心数据，无需同步全量数据，可节省大量存储空间（仅需几十GB），适合硬盘空间较小的用户，不影响钱包正常使用和安全性。
